Shader "Cartoon FX/Demo/Water Wiggle 2" {
Properties {
	...
	[Header(MoveSpeed)]
    [Toggle(USE_MOVESPEED)] _UseMoveSpeed ("Enable", Float) = 0.0
	_MovingSpeedX ("Moving Speed X", float) = 0
	_MovingSpeedY ("Moving Speed Y", float) = 0
	_MovingSpeedMultiplier ("Moving Speed Multiplier", float) = 1
...
#pragma shader_feature USE_MOVESPEED
...
#if defined(USE_MOVESPEED)
	float _MovingSpeedX;
	float _MovingSpeedY;
	float _MovingSpeedMultiplier;
#endif
...
#if defined(USE_MOVESPEED)
	IN.uv_MainTex.x += float2(_MovingSpeedX, _MovingSpeedY) * _MovingSpeedMultiplier * _Time;
#endif


	IN.uv_MainTex += float2(_OffsetX, _OffsetY);
